...
const SAVE_WARES_JSON = true;
const SAVE_MAIN_WARES = false;
...
const files = fs.readdirSync('wares');
...
for (const file of files) {
  ...
  const jObj = parser.parse(fs.readFileSync(path.join('wares', file), 'utf-8'));
  if (SAVE_WARES_JSON) {
    fs.writeFileSync(path.join('wares', file.replace('.xml', '.json')), JSON.stringify(jObj, undefined, 2));
  }
...
fs.writeFileSync('./full-wares.json', JSON.stringify(parsedWares, undefined, 2));
