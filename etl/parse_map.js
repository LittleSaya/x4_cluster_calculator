...
const SAVE_MAP = SAVE_INTERMEDIATE;
const SAVE_MAP_DEFAULTS = SAVE_INTERMEDIATE;

// 删除并重建文件夹
fs.rmSync(constants.MAP_SAVE_INTERMEDIATE, { recursive: true, force: true });
fs.mkdirSync(constants.MAP_SAVE_INTERMEDIATE);
fs.rmSync(constants.MAP_DEFAULTS_SAVE_INTERMEDIATE, { recursive: true, force: true });
fs.mkdirSync(constants.MAP_DEFAULTS_SAVE_INTERMEDIATE);
fs.rmSync(path.join(constants.MAP_SAVE_CONVERTED, 'full-map.json'), { force: true });
...
const cluster_jobjs = [];
const map_files = fs.readdirSync(constants.MAP_SAVE_RAW);
for (const map_file of map_files) {
  if (!map_file.endsWith('.xml')) {
    continue;
  }

  const jobj = parser.parse(fs.readFileSync(path.join(constants.MAP_SAVE_RAW, map_file), 'utf-8'));
  ...
  if (SAVE_MAP) {
    fs.writeFileSync(path.join(constants.MAP_SAVE_INTERMEDIATE, map_file.replace('.xml', '.json')), JSON.stringify(jobj, undefined, 2));
  }
}

const mapdefaults_jobjs = [];
const mapdefaults_files = fs.readdirSync(constants.MAP_DEFAULTS_SAVE_RAW);
for (const mapdefaults_file of mapdefaults_files) {
  if (!mapdefaults_file.endsWith('.xml')) {
    continue;
  }

  const jobj = parser.parse(fs.readFileSync(path.join(constants.MAP_DEFAULTS_SAVE_RAW, mapdefaults_file), 'utf-8'));
  mapdefaults_jobjs.push(jobj);
  if (SAVE_MAP_DEFAULTS) {
    fs.writeFileSync(path.join(constants.MAP_DEFAULTS_SAVE_INTERMEDIATE, mapdefaults_file.replace('.xml', '.json')), JSON.stringify(jobj, undefined, 2));
  }
}
...
fs.writeFileSync(path.join(constants.MAP_SAVE_CONVERTED, 'full-map.json'), JSON.stringify(parsed_map, undefined, 2));
