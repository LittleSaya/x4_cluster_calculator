...
fs.rmSync(constants.STRUCT_HABITAT_SAVE_RAW, { force: true, recursive: true });
fs.mkdirSync(constants.STRUCT_HABITAT_SAVE_RAW);
fs.rmSync(constants.STRUCT_PRODUCTION_SAVE_RAW, { force: true, recursive: true });
fs.mkdirSync(constants.STRUCT_PRODUCTION_SAVE_RAW);
fs.rmSync(constants.STRUCT_STORAGE_SAVE_RAW, { force: true, recursive: true });
fs.mkdirSync(constants.STRUCT_STORAGE_SAVE_RAW);
fs.rmSync(constants.WARES_SAVE_RAW, { force: true, recursive: true });
fs.mkdirSync(constants.WARES_SAVE_RAW);
fs.rmSync(constants.T_SAVE_RAW, { force: true, recursive: true });
fs.mkdirSync(constants.T_SAVE_RAW);
fs.rmSync(constants.MAP_SAVE_RAW, { force: true, recursive: true });
fs.mkdirSync(constants.MAP_SAVE_RAW);
fs.rmSync(constants.MAP_DEFAULTS_SAVE_RAW, { force: true, recursive: true });
fs.mkdirSync(constants.MAP_DEFAULTS_SAVE_RAW);
console.log('success');
...
for (const dat_path in sorted_file_catalog) {
  ...
  for (const file of dat) {
    ...
    if (constants.REG_STRUCT_HABITAT.test(file.file_name)) {
      save_to = path.join(constants.STRUCT_HABITAT_SAVE_RAW, path.basename(file.file_name));
      ++habitat_count;
    } else if (constants.REG_STRUCT_PRODUCTION.test(file.file_name)) {
      save_to = path.join(constants.STRUCT_PRODUCTION_SAVE_RAW, path.basename(file.file_name));
      ++production_count;
    } else if (constants.REG_STRUCT_STORAGE.test(file.file_name)) {
      save_to = path.join(constants.STRUCT_STORAGE_SAVE_RAW, path.basename(file.file_name));
      ++storage_count;
    } else if (constants.REG_WARES.test(file.file_name)) {
      save_to = path.join(constants.WARES_SAVE_RAW, '' + (++wares_count) + '_' + path.basename(file.file_name));
    } else if (constants.REG_T.test(file.file_name)) {
      save_to = path.join(constants.T_SAVE_RAW, path.basename(file.file_name));
      ++t_count;
    } else if (constants.REG_MAP.test(file.file_name)) {
      const base_name = path.basename(file.file_name);
      if (base_name === 'galaxy.xml') {
        save_to = path.join(constants.MAP_SAVE_RAW, '' + (++galaxy_count) + '_' + base_name);
      } else if (base_name.indexOf('clusters.xml') !== -1) {
        save_to = path.join(constants.MAP_SAVE_RAW, base_name);
        ++cluster_count;
      } else {
      ...
      save_to = path.join(constants.MAP_DEFAULTS_SAVE_RAW, '' + (++map_defaults_count) + '_' + path.basename(file.file_name));
    } else {
      continue;
    }
